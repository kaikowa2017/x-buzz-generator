...
export async function GET() {
  ...
  checks.push({
    ...
    detail: `✓ アカウントA: ${patternsA.length}パターン (高weight: ${patternsA.filter((p: any) => p.weight >= 2).length}件, rising: ${patternsA.filter(p => p.trend === "rising").length}件, declining: ${patternsA.filter(p => p.trend === "declining").length}件)`,
